...
import { Card, COLORS, FILLS, RowOfCards, SHAPES } from "./card-types";
...
export const findPossibleMatch = (rows: RowOfCards[]) => {
...
export const generateCards = (existingRows: RowOfCards[] = []) => {
  ...
  let hasPossibleSet = rows.length >= 4 && findPossibleMatch(rows).length > 0;

  while (!hasPossibleSet && rows.length < 10) {
    rows.push([generateCard(), generateCard(), generateCard()]);

    hasPossibleSet =
      // Check fow min rows (4) before doing more costly search for set
      4 <= rows.length && findPossibleMatch(rows).length > 0;
  }
